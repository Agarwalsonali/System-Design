...
class TomatoApp{
    public:
        ...
        void initializeRestaurants(){
            ...
            r2->addMenuItem(MenuItem("P3","Dhokla", 50));
            
            Restaurant* r3=new Restaurant("Sarvana Bhavan", "Chennai");
            r3->addMenuItem(MenuItem("P1","Masala Dosa", 90));
            r3->addMenuItem(MenuItem("P2","Idli Vada", 60));
            r3->addMenuItem(MenuItem("P3","Filter Coffee", 30));
        ...
        Order* checkout(User* user, const string& orderType, PaymentStrategy* paymentStrategy, OrderFactory* orderFactory){
            ...
            Order* order = orderFactory->createOrder(user, userCart, orderRestaurant, itemsOrdered, paymentStrategy, orderType);
